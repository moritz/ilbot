...
use IrcLog::WWW qw(http_header_obj message_line my_encode my_decode);
use utf8;
...
my $app = sub {
    ...
    my $t = HTML::Template->new(
            filename => "www/template/search.tmpl",
            global_vars => 1,
            die_on_bad_params => 0,
            default_escape => 'html',
    );
    ...
    my $nick = decode('utf8', $req->param('nick') || '');
#my $qs = decode('utf8', $req->param('q') || '');
    my $qs = $req->param('q') || '';
    $qs = my_decode($qs);
    ...
    $t->param(Q => $qs);
    my $short_channel = decode('utf8', $req->param('channel') || 'perl6');
...
#	warn join '|', @args;
    ...
    $response->body( my_encode($t->output) );
