function collapse($e) {
    ...
    $e.filter('.special.new').each(function (idx, elem) {
        ...
        if (c > 1) {
            ...
            $elem.before('<tr class="special ' + extra_class + '"><td class="nick" /> <td /><td>' + c + ' more elements. <a href="javascript:show_collapsed(\'' + $elem.attr('id') + '\')">Afficher/cacher.</a></td></tr>');
            if (uncollapse) {
...
$(document).ready(function() {
    ...
    try {
        ...
        $(filterbox).append(
            ...
            + '<p id="show_hide_all">'
            + ' <a href="javascript:show_all()">Tout afficher</a>'
            + ' <a href="javascript:hide_all()">Tout cacher</a>'
            + ' <a href="javascript:hide_nick_filter()">(cacher cette fenêtre)</a> '
            + '</p>'

            + '<div id="shown_nicks">'
            + '<h3>Pseudo affichés</h3>'
            ...
            + '<p id="filtering_off">'
            +  '<a href="javascript:filtering_off()">Désactiver le filtrage</a>'
            + '</p>'
        );
    } catch(e) { alert(e) }

    $("#"+filter_toggle_id).append(
        '<a href="javascript:filtering_on()">Activer le filtrage des pseudo</a>'
    );
...
return  '<div id="' + nick_id_prefix + nick + '" class="row">'
    + '<span class="nick"><a href="javascript:toggle_nick(\'' +nick+ '\')">'
        + nick + '</a></span>'
    + '<span class="spoken_to">(Interlocuteurs : </span>'
    + '<span class="spoken_to_show"><a href="javascript:show_spoken_to(\''
        + nick + '\')">afficher</a></span>'
...
function filtering_on() {
    $("#" +filter_toggle_id+ ">a").html("Désactiver le filtrage par pseudo");
...
function filtering_off() {
    ...
    $("#" + filter_toggle_id + ">a").html("Activer le filtrage des pseudo");
    $("#" + filter_toggle_id + ">a").attr("href", 'javascript:filtering_on()');
}

// Summary features
var summary_filter_link = '<a href="javascript:hide_non_summary()">n\'afficher que les lignes de résumé</a>';
var enable_summary_mode_html = ' <a href="javascript:ui_enable_summary_mode()">Activer le mode résumé</a>';
var summary_mode_html = '<a href="javascript:save_summary_changes()">Sauvegarder les changement du résumé</a>, <span id="toggle_summary">' + summary_filter_link + '</span> <a href="javascript:disable_summary_mode()">désactiver le mode résumé</a>';
$(document).ready(function() {
...
function save_summary_changes() {
    ...
    if (was_checked.length != 0 || newly_checked.length != 0) {
        ...
        notify('Aucun changement à sauvegarder');
    }
}
...
(function() {
    ...
    $(document).ready(function() {
        if (IlbotConfig.still_today) {
            $('#bottom').before(
                '<p id="poll">'
                +    '<input type="button" onclick="IlbotConfig.poll()" value="Rafraîchir (r)" /> '
                + '</p>'
            );
        }
